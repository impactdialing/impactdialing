class TwilioLib
  ...
  def redirect_call(call_sid, redirect_url)
    ...
    EventMachine::HttpRequest.new("https://#{@server}:80#{@root}Calls/#{call_sid}.xml").post({:head => {'authorization' => [@http_user, @http_password]},:body => {:Url => redirect_url,:Method => "POST" }})
  end
