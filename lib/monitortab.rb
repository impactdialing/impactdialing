    ...
    monitors.each do |monitor|
      monitor.account.campaigns.each do |campaign|
        ...
        caller_sessions.each do |caller_session|
          ...
          Moderator.publish_event_sync(campaign, 'update_dials_in_progress', {:campaign_id => campaign.id, :dials_in_progress => campaign.call_attempts.current_ringing_lines_count, :voters_remaining => Voter.remaining_voters_count_for('campaign_id', campaign.id)})            
        end
      end      
    end
