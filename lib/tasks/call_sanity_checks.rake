...
task :post_call_availabilty_check => :environment do
  ...
  sql="select cs.id, now()-ca.call_end as seconds_since_last_attempt_ended, c.name, ca.call_end, attempt_in_progress, cs.campaign_id from caller_sessions cs
  join call_attempts ca on ca.id=cs.attempt_in_progress
  join campaigns c on c.id=cs.campaign_id
   where on_call=1 and available_for_call=0 and now()-ca.call_end > 90"
...
#    puts @session.inspect
    ...
    if @session.campaign.predective_type=="preview"
