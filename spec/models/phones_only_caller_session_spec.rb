  ...
  describe "ready_to_call" do
    ...
    describe "time period exceeded" do
      ...
      it "should render correct twiml" do
        ...
        caller_session.ready_to_call.should eq("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Response><Say>You can only call this campaign between 9 AM and 9 PM. Please try back during those hours.</Say><Hangup/></Response>")
      end
  ...
  describe "voter_response" do
    ...
    describe "more_questions_to_be_answered" do
      ...
      it "should move to read_next_question state" do
        call_attempt = Factory(:call_attempt, voter: @voter)
        caller_session = Factory(:phones_only_caller_session, caller: @caller, on_call: false, available_for_call: false, campaign: @campaign, state: "voter_response", voter_in_progress: @voter, question_id: @question.id, attempt_in_progress: call_attempt, script_id: @script.id)
        RedisCallerSession.set_request_params(caller_session.id, {digit: 1, question_number: 0, question_id: 1})
