...
describe CallAttempt do
  ...
  describe "voter connected" do
    ...
    it "conferences a call_attempt to a caller_session" do
      ...
      Moderator.stub!(:publish_event).with(session.caller, 'voter_connected', {:campaign_id => campaign.id, :caller_id => session.caller.id, :dials_in_progress => 0})
    ...
    it "plays a recorded message to the voters answering machine and hangs up" do
      ...
      voter = Factory(:voter, :campaign => campaign)
      call_attempt = Factory(:call_attempt, :voter => voter, :campaign => campaign)
      call_attempt.play_recorded_message.should == Twilio::Verb.new { |v|
    ...
    it "disconnects the voter from the caller" do
      ...
      time_now = Time.now
      Moderator.stub!(:publish_event).with(caller_session.caller, 'voter_disconnected', {:campaign_id => call_attempt.campaign.id,
        :caller_id => caller_session.caller.id, :dials_in_progress => 0, :voters_remaining => 0})
  ...
  describe "Pusher" do
    ...
    it "pushes voter details" do
      ...
      Moderator.stub!(:publish_event).with(session.caller, 'voter_connected', {:campaign_id => campaign.id, 
    ...
    it "pushes 'voter_disconnected' event when a call_attempt ends" do
      ...
      caller_session = Factory(:caller_session, :caller => Factory(:caller), :campaign => Factory(:campaign, :use_web_ui => true))
      attempt = Factory(:call_attempt, :voter => voter, :caller_session => caller_session)
      channel = mock
      Moderator.should_receive(:publish_event).with(caller_session.caller, 'voter_disconnected', {:campaign_id => attempt.campaign.id,
         :caller_id => caller_session.caller.id, :dials_in_progress => 0, :voters_remaining=>0})
