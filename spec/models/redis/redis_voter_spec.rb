...
describe RedisVoter do
  ...
  it "should redis voter" do
    ...
    RedisVoter.load_voter_info(voter.id, voter, @redis)
    RedisVoter.read(voter.id, @redis).should eq({"CustomID"=>"", "Email"=>"", "FirstName"=>"abc", "LastName"=>"",
  ...
  describe "abandon_call" do
    ...
    it "should change status" do
      ...
      RedisVoter.read(voter.id, @redis)['status'].should eq(CallAttempt::Status::ABANDONED)  
    end
    
    it "should set callback to false" do
      voter = Factory(:voter)
      RedisVoter.abandon_call(voter.id, @redis)
      RedisVoter.read(voter.id, @redis)['call_back'].should eq("false")  
    end

    it "should set caller session to nil" do
      voter = Factory(:voter)
      RedisVoter.abandon_call(voter.id, @redis)
      RedisVoter.read(voter.id, @redis)['caller_session_id'].should be_nil
    end

    it "should set caller  to nil" do
      voter = Factory(:voter)
      RedisVoter.abandon_call(voter.id, @redis)
      RedisVoter.read(voter.id, @redis)['caller_id'].should be_nil
    end
  ...
  describe "end_answered_call" do
    ...
    it "should set last call attempt time" do
      ...
      RedisVoter.read(voter.id, @redis)['last_call_attempt_time'].should_not be_nil 
    end
  ...
  describe "answered_by_machine" do
    ...
    it "should set status" do
      ...
      RedisVoter.read(voter.id, @redis)['status'].should eq("status")      
    end
  ...
  describe "set status" do
    ...
    it "should set status" do
      voter = Factory(:voter)
      RedisVoter.set_status(voter.id, "status", @redis)
      RedisVoter.read(voter.id, @redis)['status'].should eq("status")      
    end
  ...
  describe "schedule_for_later" do
    ...
    it "should set status" do
      voter = Factory(:voter)
      RedisVoter.schedule_for_later(voter.id, Time.now, @redis)
      RedisVoter.read(voter.id, @redis)['status'].should eq(CallAttempt::Status::SCHEDULED)      
    end

    it "should set scheduled_date" do
      voter = Factory(:voter)
      RedisVoter.schedule_for_later(voter.id, Time.now, @redis)
      RedisVoter.read(voter.id, @redis)['scheduled_date'].should eq(Time.now.to_s)      
    end

    it "should set callback to true" do
      voter = Factory(:voter)
      RedisVoter.schedule_for_later(voter.id, Time.now, @redis)
      RedisVoter.read(voter.id, @redis)['call_back'].should eq("true")      
    end
  ...
  describe "assigned_to_caller?" do
    ...
    it "should check id caller is assigned to voter" do
      RedisVoter.assign_to_caller(1, 1, @redis)
      RedisVoter.assigned_to_caller?(1, @redis).should eq(true)
    end
  ...
  describe "connect_lead_to_caller" do
    ...
    it "should set caller id when caller session already assigned to voter" do
      ...
      RedisVoter.assign_to_caller(1, caller_session.id, @redis)
      RedisVoter.should_receive(:assigned_to_caller?).and_return(true)
      RedisVoter.connect_lead_to_caller(1, 1, 1, @redis)
      RedisVoter.read(1, @redis)["caller_id"].should eq("1")
    end
    ...
    it "should set status as inprogress when caller session already assigned to voter" do
      ...
      RedisVoter.should_receive(:assigned_to_caller?).and_return(true)
      RedisVoter.connect_lead_to_caller(1, 1, 1, @redis)
      RedisVoter.read(1, @redis)["status"].should eq(CallAttempt::Status::INPROGRESS)
    end
    ...
    it "should set caller as longest available if not already assigned" do
      ...
      RedisVoter.should_receive(:assign_to_caller).with(1, caller_session.id, @redis) 
      RedisAvailableCaller.should_receive(:remove_caller).with(1, caller_session.id, @redis)      
      RedisVoter.connect_lead_to_caller(1, 1, 1, @redis)
      RedisVoter.read(1, @redis)["caller_id"].should eq("2")
      RedisVoter.read(1, @redis)["status"].should eq(CallAttempt::Status::INPROGRESS)
    end
