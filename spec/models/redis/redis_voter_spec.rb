...
describe RedisVoter do
  ...
  describe "assigned_to_caller?" do
    ...
    it "should check id caller is assigned to voter" do
      ...
      RedisVoter.assigned_to_caller?(voter.id).should eq(true)
    end
  ...
  describe "connect_lead_to_caller" do
    ...
    it "should set caller id when caller session already assigned to voter" do
      ...
      RedisVoter.read(voter.id)["caller_id"].should eq(caller.id.to_s)
    end
    ...
    it "should set status as inprogress when caller session already assigned to voter" do
      ...
      RedisCaller.should_not_receive(:longest_waiting_caller)
      RedisVoter.connect_lead_to_caller(voter.id, voter.campaign.id, call_attempt.id)
      RedisVoter.read(voter.id)["status"].should eq(CallAttempt::Status::INPROGRESS)
    end
    ...
    it "should set caller as longest available if not already assigned" do
      ...
      voter = Factory(:voter, campaign: campaign)
      caller = Factory(:caller)
      caller_session = Factory(:caller_session, caller: caller, campaign: campaign, available_for_call: true, on_call: true)
      call_attempt = Factory(:call_attempt, campaign: campaign, voter: voter)
  ...
  describe "could_not_connect_to_available_caller?" do
    ...
    it "should return true if caller is assigned to voter but caller is disonnected" do
      ...
      caller_session = Factory(:caller_session, caller_id: caller.id, available_for_call: false, on_call: false, campaign: campaign)
      RedisCallerSession.load_caller_session_info(caller_session.id, caller_session)
      RedisCaller.add_caller(campaign.id, caller_session.id)
      RedisCaller.disconnect_caller(campaign.id, caller_session.id)
      voter_list = Factory(:voter_list, campaign: campaign)
      voter = Factory(:voter, voter_list: voter_list, FirstName: "abc", caller_session: caller_session, campaign: campaign)
      RedisVoter.load_voter_info(voter.id, voter)      
      RedisVoter.assign_to_caller(voter.id, caller_session.id)
      RedisVoter.could_not_connect_to_available_caller?(voter.id, campaign.id).should be_true
    end
    
    it "should return false if caller is assigned to voter but caller is connected" do
      campaign = Factory(:campaign)
      caller = Factory(:caller, campaign: campaign)
      caller_session = Factory(:caller_session, caller_id: caller.id, available_for_call: true, on_call: true, campaign: campaign)
      RedisCaller.add_caller(campaign.id, caller_session.id)
      RedisCallerSession.load_caller_session_info(caller_session.id, caller_session)
      voter_list = Factory(:voter_list, campaign: campaign)
      voter = Factory(:voter, voter_list: voter_list, FirstName: "abc", campaign: campaign)
      RedisVoter.load_voter_info(voter.id, voter)      
      RedisVoter.assign_to_caller(voter.id, caller_session.id)
      RedisVoter.could_not_connect_to_available_caller?(voter.id, campaign.id).should be_false
    end
