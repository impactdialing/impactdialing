...
describe PossibleResponse do
  ...
  it "returns the calculated percentage value for possible response" do
    ...
    answer = Factory(:answer, :voter => Factory(:voter, :campaign => campaign),:possible_response => possible_response, :question => Factory(:question, :script => Factory(:script)), :created_at => now)
