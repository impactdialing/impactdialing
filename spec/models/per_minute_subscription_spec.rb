...
describe PerMinuteSubscription do
    describe "campaign_types" do
    it "should return preview and power modes" do
     PerMinuteSubscription.new.campaign_types.should eq([Campaign::Type::PREVIEW, Campaign::Type::POWER, Campaign::Type::PREDICTIVE])
    end
  end

  describe "campaign_type_options" do
    it "should return preview and power modes" do
     PerMinuteSubscription.new.campaign_type_options.should eq([[Campaign::Type::PREVIEW, Campaign::Type::PREVIEW], [Campaign::Type::POWER, Campaign::Type::POWER],[Campaign::Type::PREDICTIVE, Campaign::Type::PREDICTIVE]])
    end
  end
  describe "campaign" do
    let(:account) { create(:account, subscription_name: Account::Subscription_Type::PER_MINUTE) }
    ...
    it "should not allow preview dialing mode for per minute subscription" do
      campaign = build(:preview, account: account)
      campaign.save.should be_true
    end

    it "should not allow power dialing mode for per minute subscription" do
  ...
  describe "transfer_types" do
    it "should return warm and cold transfers" do
      PerMinuteSubscription.new.transfer_types.should eq([Transfer::Type::WARM, Transfer::Type::COLD])
    end
  end

  describe "transfers" do
    let(:account) { create(:account, subscription_name: Account::Subscription_Type::PER_MINUTE, record_calls: false) }
  ...
  describe "caller groups" do
    describe "caller_groups_enabled?" do
      it "should say enabled" do
        PerMinuteSubscription.new.caller_groups_enabled?.should be_true
      end
    end
  ...
  describe "call recordings" do
    describe "call_recording_enabled?" do
      it "should say  enabled" do
        PerMinuteSubscription.new.call_recording_enabled?.should be_true
      end
    end
