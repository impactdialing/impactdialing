...
describe CsvValidator do
  ...
  describe "headers are repeated" do
    ...
    it "should set headers repeated error message" do
      ...
      expect(csv_validator.errors).to eql ([I18n.t('csv_validator.duplicate_headers', :duplicate_headers => "PHONENUMBER")])
    end
  end
  ...
  describe "duplicate headers and no row data" do
    ...
    it "should set both duplicate headers and no row data messages" do
      csv_validator = CsvValidator.new(csv_file)
      expect(csv_validator.errors).to eql ([(I18n.t('csv_validator.missing_header_or_rows')), (I18n.t('csv_validator.duplicate_headers', :duplicate_headers => "PHONENUMBER"))])
    end
  end
end
