  ...
  describe "initial" do
    ...
    describe "incoming call answered by human" do
      ...
      it "should move to the connected state" do
        ...
        @call_attempt.should_receive(:enqueue_call_flow).with(CallPusherJob, [{call_attempt_id: @call_attempt.id, event: "publish_voter_connected"}])
        @call_attempt.should_receive(:enqueue_moderator_flow).with(ModeratorCallJob, [{call_attempt_id: @call_attempt.id, event:"publish_voter_event_moderator"}])
        call.incoming_call!
  ...
  describe "connected" do
     ...
     describe "disconnect"  do
       ...
       it "should update call attempt recording_duration" do
         ...
         @call_attempt.should_receive(:enqueue_call_flow).with(CallPusherJob, [{call_attempt_id: @call_attempt.id, event: "publish_voter_disconnected"}])
         @call_attempt.should_receive(:enqueue_moderator_flow).with(ModeratorCallJob, [{call_attempt_id: @call_attempt.id, event: "publish_voter_event_moderator"}])
         call.disconnect!
  ...
  describe "call_answered_by_lead" do
    describe "submit_result" do
      ...
      it "should wrapup call_attempt" do
        ...
        @call_attempt.should_receive(:enqueue_call_flow).with(RedirectCallerJob, [{caller_session_id: @caller_session.id}])
        @call_attempt.should_receive(:enqueue_moderator_flow).with(ModeratorCallJob, [{call_attempt_id: @call_attempt.id, event: "publish_voter_event_moderator"}])
        call.submit_result!
        call.call_attempt.wrapup_time.should_not be_nil
      end
