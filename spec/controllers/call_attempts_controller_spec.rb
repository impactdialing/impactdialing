...
describe CallAttemptsController do
  ...
  describe "gathering responses" do
    ...
    it "sends next voter to be dialed via voter_push Pusher event" do
      ...
      Pusher.should_receive(:[]).with(anything).and_return(channel)
      channel.should_receive(:trigger_async).with("voter_push", info.merge({dialer: next_voter.campaign.predictive_type}))
  ...
  describe "calling in" do
    ...
    it "notifies pusher when a call attempt is connected" do
      ...
      pusher_session = mock
      pusher_session.should_receive(:trigger_async).with('voter_connected', {:attempt_id=> call_attempt.id, :voter => voter.info}.merge(:dialer => campaign.predictive_type))
    ...
    it "notifies pusher when a call attempt is answered by a machine." do
      ...
      info[:fields]['status'] = CallAttempt::Status::READY
      Pusher.stub(:[]).with(session_key).and_return(pusher_session)
      pusher_session.should_receive(:trigger_async).with("answered_by_machine", {:dialer=>"preview"})
      pusher_session.should_receive(:trigger_async).with('voter_push', info.merge(:dialer => campaign.predictive_type))
      pusher_session.should_receive(:trigger_async).with('conference_started', {:dialer=>"preview"})
      post :connect, :id => call_attempt.id, :AnsweredBy => "machine"
