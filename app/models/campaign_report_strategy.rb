class CampaignReportStrategy
  ...
  def csv_for_call_attempt(call_attempt)
    ...
    voter_fields = voter.selected_fields(@selected_voter_fields.try(:compact))
    custom_fields = voter.selected_custom_fields(@selected_custom_voter_fields)
    [voter_fields, custom_fields, call_attempt_details(call_attempt, voter)].flatten
  end
  ...
  def call_attempt_info(call_attempt)
    if @mode == CampaignReportStrategy::Mode::PER_LEAD
      [caller_name(call_attempt), CampaignReportStrategy.map_status(call_attempt.status), time_dialed(call_attempt), 
       time_answered(call_attempt), time_ended(call_attempt), number_of_attempts(call_attempt.voter), 
       call_attempt.try(:report_recording_url)].flatten
    else
      [caller_name(call_attempt), CampaignReportStrategy.map_status(call_attempt.status), time_dialed(call_attempt), 
       time_answered(call_attempt), time_ended(call_attempt), call_attempt.try(:report_recording_url)].flatten      
    end
  ...
  def time_answered(call_attempt)
    call_attempt.try(:connecttime).try(:in_time_zone, @campaign.time_zone)
  end
