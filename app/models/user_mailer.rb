class UserMailer
  ...
  def voter_list_upload(response,user_domain,email, list_name)
    unless response['success'].blank?
      subject = I18n.t(:voter_list_upload_succeded_subject, :list_name, list_name)
      content = response['success'].join("<br/>")
    else
      subject = I18n.t(:voter_list_upload_failed_subject, :list_name, list_name)
  ...
  def deliver_download_failure(user, job, exception)
    ...
    @uakari.send_email({ :message => { :subject => subject, :text => exception_content, :html => exception_content, :from_name => white_labeled_title(user.domain), :from_email => 'email@impactdialing.com', :to_email => ['aninda@c42.in','nikhil@activesphere.com','michael@impactdialing.com','brian@impactdialing.com']} })
  end
end
