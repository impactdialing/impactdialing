class Caller < ActiveRecord::Base
  ...
  def answered_call_stats(from, to, campaign)
    ...
    total_answers = Answer.find_by_sql("SELECT COUNT(question_id) AS count_question_id, question_id AS question_id  FROM answers USE INDEX (index_answers_count_question_id)  WHERE (campaign_id = #{campaign.id} and caller_id = #{self.id}) AND (created_at BETWEEN '#{from.to_datetime.utc}' AND '#{to.to_datetime.utc}') GROUP BY question_id ")
