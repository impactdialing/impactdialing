class AccountUsage
  ...
  def billable_usage
    ...
    lead_times =  CallAttempt.from('call_attempts use index (index_call_attempts_on_campaign_id_created_at_status)').
      ...
      group("campaign_id").sum('ceil(TIMESTAMPDIFF(SECOND ,tStartTime,tEndTime)/60)')
    transfer_times = TransferAttempt.where("campaign_id in (?)",@campaign_ids).between(@from_date, @to_date).group("campaign_id").sum('ceil(TIMESTAMPDIFF(SECOND ,tStartTime,tEndTime)/60)')
  ...
  def callers_billable_usage
    ...
    lead_times = CallAttempt.where("caller_id in (?)",@caller_ids).between(@from_date, @to_date).group("caller_id").sum('ceil(TIMESTAMPDIFF(SECOND ,tStartTime,tEndTime)/60)')
    total_times = {}      
  ...
  def callers_status_times
   CallAttempt.from('call_attempts use index (index_call_attempts_on_campaign_id_created_at_status)').
     ...
     sum('ceil(TIMESTAMPDIFF(SECOND ,tStartTime,tEndTime)/60)')
  end
