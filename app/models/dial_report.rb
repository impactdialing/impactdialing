class DialReport
  ...
  def compute_campaign_report(campaign, from_date, to_date)
    ...
    @scheduled_for_now = @campaign.all_voters.scheduled
    @abandoned_count = @campaign.all_voters.by_status(CallAttempt::Status::ABANDONED)
    @voters_not_available_for_retry = @campaign.all_voters.not_avialable_to_be_retried(@campaign.recycle_rate).count
    @voters_available_for_retry = @campaign.all_voters.avialable_to_be_retried(@campaign.recycle_rate)
    overview_summary
    per_lead_dials
    per_attempt_dials
  end
