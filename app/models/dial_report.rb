class DialReport
  ...
  def compute_campaign_report(campaign, from_date, to_date)
    ...
    @to_date = to_date
    @campaign = campaign
    @leads_grouped_by_status = @campaign.all_voters.group("status").count('id')
    @leads_grouped_by_status_filtered = @campaign.all_voters.last_call_attempt_within(@from_date, @to_date).group("status").count('id')
    @attempts_grouped_by_status_filtered = @campaign.call_attempts.between(@from_date, @to_date).group("status").count('id')
