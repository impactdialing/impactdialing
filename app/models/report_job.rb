class ReportJob < Struct.new(:campaign, :user, :selected_voter_fields, :selected_custom_voter_fields, :download_all_voters, :from_date, :to_date)
  ...
  def perform
    ...
    @report = CSV.generate do |csv|
      csv << [selected_voter_fields.compact, selected_custom_voter_fields, "Caller", "Status", "Call start", "Call end", "Attempts", "Recording", @campaign_questions.collect { |q| q.text }, @campaign_notes.collect { |note| note.note }].flatten
