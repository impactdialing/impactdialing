...
class CallAttempt < ActiveRecord::Base
  ...
  def disconnect(params={})
    ...
    Moderator.publish_event(session.caller, 'voter_disconnected', {:campaign_id => campaign.id, :dials_in_progress => campaign.caller_sessions.dial_in_progress.length})
    hangup
  end

  def fail
    caller_session.publish('voter_push',self.campaign.next_voter_in_dial_queue.info) if caller_session && campaign.predictive_type == Campaign::Type::PREVIEW
