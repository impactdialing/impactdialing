...
class CallerSession < ActiveRecord::Base
  ...
  def reassign_caller_session_to_campaign
    ...
    Moderator.publish_event(campaign, "caller_re_assigned_to_campaign", {:caller_id => caller.id, :campaign_fields => {:id => campaign.id, :campaign_name => campaign.name, :callers_logged_in => campaign.caller_sessions.on_call.length+1,
