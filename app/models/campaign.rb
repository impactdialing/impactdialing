...
public
  ...
  def per_status_counts(from_date=nil, to_date=nil)
    all_voters.select('status').group("status").count
  end

  def dialed_and_complete_count(from_date=nil, to_date=nil)
    all_voters.where({
      status: CallAttempt::Status.completed_list(self),
      call_back: false
    }).count
  end

  def dialed_and_complete_percent(from_date=nil, to_date=nil)
    number = dialed_and_complete_count
    percent_of_all_voters(number)
  end

  def not_dialed_count(from_date=nil, to_date=nil)
    all_voters.where(status: CallAttempt::Status.not_dialed_list).count
  end

  def not_dialed_percent(from_date=nil, to_date=nil)
    number = not_dialed_count(from_date, to_date)
    percent_of_all_voters(number)
  end

  def dialed_and_available_for_retry_count(from_date=nil, to_date=nil)
    all_voters.available_for_retry(self).count
  end

  def dialed_and_available_for_retry_percent(from_date=nil, to_date=nil)
    number = dialed_and_available_for_retry_count(from_date, to_date)
    percent_of_all_voters(number)
  end

  def dialed_and_not_available_for_retry_count(from_date=nil, to_date=nil)
    all_voters.not_available_for_retry(self).count
  end

  def dialed_and_not_available_for_retry_percent(from_date=nil, to_date=nil)
    number = dialed_and_not_available_for_retry_count(from_date, to_date)
    percent_of_all_voters(number)
  end
end
