...
public
  ...
  def household_query
    Household.where(campaign_id: @campaign.id).where("households.status NOT IN (?)", [CallAttempt::Status::RINGING,CallAttempt::Status::READY,"not called"]).includes(:call_attempts, :voters)
  end
