...
public
  def update_or_create_customer_and_card(email, token)
    if account.billing_provider_customer_id.present?
      ...
      Rails.logger.debug "StripeEvent: CreditCard update: #{gateway_customer.id}; with: #{email}, #{token}"
    else
      ...
      Rails.logger.debug "StripeEvent: CreditCard create: #{gateway_customer.id}; update: #{account.id}; with: #{email}, #{token}"
    end
  end
end
