class VoterList < ActiveRecord::Base
  ...
  def append_from_csv(csv_to_system_map, csv_file, seperator)
    result      = {:messages     => [],
                   :successCount => 0,
                   :failedCount  => 0}
    voters_list = FasterCSV.parse(csv_file.read, :col_sep => seperator)
    ...
    voters_list.each_with_index do |voter_info, row|
      ...
      if existing_voter_entry.present?
        ...
        voter          = Family.new
        voter.voter_id = existing_voter_entry.id
      ...
      voter.voter_list_id = self.id
      voter.user_id       = self.user_id
      voter.campaign_id   = self.campaign_id

      csv_headers.each_with_index do |csv_column_title, column_location|
        system_column = csv_to_system_map[csv_column_title]
        voter[system_column] = voter_info[column_location] if system_column
      end

      unless voter.save
        result[:messages] << "Row #{(row+2).to_s} : #{voter.errors.full_messages.join("; ")}"
