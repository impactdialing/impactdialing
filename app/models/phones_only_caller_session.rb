  ...
  call_flow :state, :initial => :initial do    
      ...
      state :read_choice do     
        ...
        response do |xml_builder, the_call|
          xml_builder.Gather(:numDigits => 1, :timeout => 10, :action => flow_caller_url(caller, session:  self, event: 'read_instruction_options' ,:host => Settings.twilio_callback_host, :port => Settings.twilio_callback_port), :method => "POST", :finishOnKey => "5") do
      ...
      state :ready_to_call do  
        ...
        response do |xml_builder, the_call|
          xml_builder.Redirect(flow_caller_url(self.caller, event: 'start_conf', :host => Settings.twilio_callback_host, :port => Settings.twilio_callback_port, :session => id))          
        end        
      ...
      state :instructions_options do   
        ...
        response do |xml_builder, the_call|
          ...
          xml_builder.Redirect(flow_caller_url(self.caller, event: 'callin_choice', :host => Settings.twilio_callback_host, :port => Settings.twilio_callback_port, :session => id))          
        end        
      end
      ...
      state :reassigned_campaign do
        ...
        response do |xml_builder, the_call|
          ...
          xml_builder.Redirect(flow_caller_url(self.caller, event: 'callin_choice', :host => Settings.twilio_callback_host, :port => Settings.twilio_callback_port, :session => id, :Digits => "*"))
        end
      end
      ...
      state :conference_started_phones_only do
        before(:always) {start_conference; enqueue_call_flow(PreviewPowerDialJob, [caller_session_id: self.id, voter_id: voter_in_progress.id])}
      ...
      state :skip_voter do
        ...
        response do |xml_builder, the_call|
          xml_builder.Redirect(flow_caller_url(self.caller, event: 'skipped_voter', :host => Settings.twilio_callback_host, :port => Settings.twilio_callback_port, :session => id))          
        end        
      ...
      state :wrapup_call do
        ...
        response do |xml_builder, the_call|
          xml_builder.Redirect(flow_caller_url(self.caller, event: 'next_call', :host => Settings.twilio_callback_host, :port => Settings.twilio_callback_port, :session => id))          
        end        
