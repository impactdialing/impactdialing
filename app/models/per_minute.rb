class PerMinute < Subscription
  ...
  def subscribe(upgrade=true, old_subscription=nil)
    self.total_allowed_minutes = calculate_minutes_on_upgrade
    self.import_available_minutes(old_subscription)
  end

  def import_available_minutes(old_subscription=nil)
    return if old_subscription.nil?

    remaining = old_subscription.available_minutes
    if remaining > 0
      old_subscription.zero_minutes!
      self.total_allowed_minutes = remaining + self.total_allowed_minutes
    end
  end
  ...
  def self.recharge_subscription(account_id, amount)
    ...
    new_subscription = PerMinute.new({
    ...
    new_subscription.subscribe(true, subscription)
