module Client
  class ReportsController < ClientController
    ...
    def dials
      ...
      @campaign.all_voters.last_call_attempt_within(@from_date, @to_date)
      dialed_voters_ids = Voter.find(:select=>"id",:conditions => [ "campaign_id = ? and last_call_attempt_time between  ? and ?", @campaign.id, from, (to + 1.day)])
