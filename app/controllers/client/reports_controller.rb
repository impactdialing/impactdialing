module Client
  class ReportsController < ClientController
    ...
    def set_date_range
      ...
      @from_date = from_date || (@campaign.call_attempts.first.try(:created_at) || Time.now).in_time_zone(@campaign.time_zone).to_date
      @to_date = to_date || (@campaign.call_attempts.last.try(:created_at) || Time.now).in_time_zone(@campaign.time_zone).to_date
    end
