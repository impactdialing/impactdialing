module TimeZoneHelper
  ...
  def set_date_range(campaign, from_date, to_date)
    ...
    begin
      formatted_from_date = Time.strptime("#{from_date} #{time_zone.formatted_offset}", "%m/%d/%Y %:z") if from_date
      formatted_to_date = Time.strptime("#{to_date} #{time_zone.formatted_offset}", "%m/%d/%Y %:z") if to_date
    rescue Exception => e
