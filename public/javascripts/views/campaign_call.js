  ...
  fetchCallerInfo: function(){
    ...
    $.ajax({
      ...
      success: function(data){
        self.model.set(data);
        self.pusher = self.model.get("pusher");
        ...
        $("#callin").show();
        if (self.model.get("env") != "integration_test" &&( !FlashDetect.installed || !flash_supported() || !browser_supported())){
          $("#start_calling").hide();
        }
  ...
  callerShouldNotDial:  function(error){
   ...
   setupTwilio:  function(){
    ...
    Twilio.Device.connect(function (conn) {
      if(self.model.get("env") != "integration_test"){
        $("#start_calling").hide();
      }
