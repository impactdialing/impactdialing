...
class UpdateTwilioStatsCallerSession
  ...
  def self.perform
    ...
    WebuiCallerSession.where("endtime is not NULL and tPrice is NULL and (tStatus is NULL or tStatus = 'completed') and service_provider != 'voxeo'").limit(100).each do |session|
    ...
    PhonesOnlyCallerSession.where("endtime is not NULL and tPrice is NULL and (tStatus is NULL or tStatus = 'completed') and service_provider != 'voxeo'").limit(100).each do |session|
        caller_sessions << twillio_lib.update_twilio_stats_by_model(session)
    end
