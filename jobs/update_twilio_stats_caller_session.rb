...
class UpdateTwilioStatsCallerSession
  ...
  def self.perform
    ...
    WebuiCallerSession.where("tPrice is NULL and (tStatus is NULL or tStatus = 'completed')").limit(100).each do |session|
    ...
    PhonesOnlyCallerSession.where("tPrice is NULL and (tStatus is NULL or tStatus = 'completed')").limit(5000).each do |session|
        caller_sessions << twillio_lib.update_twilio_stats_by_model(session)
    end
