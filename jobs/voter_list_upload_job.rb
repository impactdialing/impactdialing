...
class VoterListUploadJob 
  ...
  @queue = :voter_list_upload_worker_job
   ...
   def self.perform(voter_list_id, email, domain, callback_url, strategy="webui")
     ...
     job = VoterListJob.new(voter_list.separator, HashWithIndifferentAccess.new(voter_list.csv_to_system_map), voter_list.s3path, voter_list.name, voter_list.campaign_id, voter_list.account_id, domain, email, callback_url, strategy="webui")
     job.perform
   end
