...
class UpdateStatsEmAbandoned
  ...
  def self.perform
    ...
    call_attempts = CallAttempt.where("status in (?) and tPrice is NULL and (tStatus is NULL or tStatus = 'completed') and sid is not null", ['Call abandoned']).limit(500)
      EM.synchrony do
        concurrency = 500
