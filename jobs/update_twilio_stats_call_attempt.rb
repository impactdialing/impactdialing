...
class UpdateTwilioStatsCallAttempt
  ...
  def self.perform
    ...
    CallAttempt.where("status in (?) and tPrice is NULL and (tStatus is NULL or tStatus = 'completed') and sid is not null", ['Call abandoned', 'Call completed with success.', 'Message delivered', 'Hangup or answering machine']).limit(1000).each do |attempt|
